...
retract_board(Piece, Board, NewBoard, [X,Y2,X,Y1]) :-
      ...
      YOpponent is Y2 - 1,
      get_piece(Board, X, YOpponent, OpponentPiece),  
      jumper_piece(OpponentPiece, Jumper), retract_piece_type(OpponentPiece, X, YOpponent),
...
simulate_move([Player|Board], [X1,Y1,X2,Y2], Value) :- % ESTÁ A FALHAR EM SITUAÇÃO DE JUMP!!
      (
            ...
            value([Player|NewBoard], Value), write('Piece name: '), write(Piece),write(' Move: '), write([X1,Y1,X2,Y2]),
            retract_board(Piece, NewBoard, _, [X1,Y1,X2,Y2]), write('   Havent died'), nl,
...
level_current_player(_, 11, PlayerLevel) :-
      PlayerLevel = 1.
